# Dashboard Admin Tab Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add an inline Dashboard tab to the admin page that shows the teams grid and remaining players pane using the admin's existing AuctionContext state.

**Architecture:** Extract the dashboard display components from `DashboardPage.jsx` into a new `DashboardView.jsx` shared component. AdminPage imports and renders it in a new "Dashboard" tab. DashboardPage is updated to use the same shared component, eliminating duplication.

**Tech Stack:** React 18, Vite, Socket.io (admin already connected — no second socket needed for the tab)

---

### Task 1: Create `DashboardView.jsx`

**Files:**
- Create: `client/src/components/admin/DashboardView.jsx`

Extract all display logic from `DashboardPage.jsx` into this component. It receives `state` as a prop.
...
```bash
git add client/src/components/admin/DashboardView.jsx
git commit -m "feat: extract DashboardView shared component"
```

---

### Task 2: Update `DashboardPage.jsx` to use `DashboardView`

**Files:**
- Modify: `client/src/pages/DashboardPage.jsx`

Replace the inline content area + all helper function definitions with the imported `DashboardView`.
...
**Step 2: Replace the return statement's content area**

The `return` in `DashboardPage` (starting at line 177) currently has a `{currentPlayer && ...}` banner and a main content `<div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>` block. Replace those two sections with `<DashboardView state={state} />`:

Before (lines ~219–296):
```jsx
...
Also remove the local variable declarations that are now only used by the removed JSX (lines ~166–175):
```jsx
  // DELETE these lines — DashboardView handles them internally:
  const teamList = Object.values(teams).sort((a, b) => a.name.localeCompare(b.name));
  const [rightWidth, setRightWidth] = useState(380);
  const startDragRight = useCallback(...);
```

Keep: `phase`, `teams`, `leagueConfig`, `players`, `currentPlayerIndex`, `startingBudget`, `squadSize`, `currentPlayer`, `totalSold`, `totalUnsold`, `totalPending` — all still used in the header.

**Step 3: Remove all now-redundant helper functions** (lines ~302–607):
- `RemainingPlayersPane`
- `TeamCard`
- `poolColor`
- `DColHead`
- `DCell`
- `Stat`
- `PhaseChip` — keep this one, it's still used in the DashboardPage header
- `fmtPts` — keep this one, it's still used in the DashboardPage header stats? Actually check: `fmtPts` is not used in the header directly (header uses raw numbers). Remove it.

Actually `PhaseChip` IS used in the header at line ~193. Keep it in DashboardPage.jsx.

**Step 4: Commit**

```bash
git add client/src/pages/DashboardPage.jsx
git commit -m "refactor: DashboardPage uses shared DashboardView component"
```

---

### Task 3: Add Dashboard tab to `AdminPage.jsx`
...
**Step 2: Add tab to TABS array (line 15)**

```jsx
const TABS = ['League Setup', 'Auction Controls', 'Teams & Rosters', 'Player Data', 'Settings', 'Dashboard'];
```

**Step 3: Change tab content rendering to handle Dashboard's full-height layout**

The current tab content block (lines ~114–131) is:
```jsx
          {/* Tab content */}
...
git commit -m "feat: add Dashboard tab to admin page"
```
...
**Step 1:** Run `npm run dev` and open `http://localhost:5173/admin`

**Step 2:** Confirm the "Dashboard" tab appears and shows the teams grid + remaining players pane.

**Step 3:** Confirm the `/dashboard` public page still works correctly.

**Step 4:** Confirm the "On Block" live banner appears in the Dashboard tab when a round is LIVE.
