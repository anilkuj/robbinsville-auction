...
**Extra CSV columns:** Any CSV columns beyond `name` and `pool` are captured as `player.extra` (object). Extra data flows into roster entries on sale. Displayed in: AuctionPage right panel (remaining players) and the Player Data admin tab. Roster views (Sidebar, Dashboard team cards, mobile roster) intentionally show only Name, Pool, and Price — extra fields are hidden there.

**Player Data tab column order:** `# | Player Name | Pool | Status | Sold To | Sold Price | extras | Base | Edit`
